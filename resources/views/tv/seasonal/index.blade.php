...
@section('content')
  <div class="seasonal">
    <div class="banner is-{{ $currentSeason->slug }}">
      <div class="container">
        <img class="banner__image" src="/images/banner-{{ $currentSeason->slug }}.svg">
      </div>
          ...
          <div class="filters__seasons">
            <ul class="seasons__list">
              @foreach ($seasons['before'] as $seasonBefore)
                <li><a href="{{ route('television.seasonal.show', [$seasonBefore->year, $seasonBefore->slug]) }}">{{ $seasonBefore->slug }} {{ $seasonBefore->year }}</a></li>
              @endforeach
              <li class="is-active" id="currentSeason" data-season="{{ $currentSeason->id }}"><a class="text-{{ $currentSeason->slug }}" href="{{ route('television.seasonal.show', [$currentSeason->year, $currentSeason->slug]) }}">{{ $currentSeason->slug }} {{ $currentSeason->year }}</a></li>
              @foreach ($seasons['after'] as $seasonAfter)
                <li><a href="{{ route('television.seasonal.show', [$seasonAfter->year, $seasonAfter->slug]) }}">{{ $seasonAfter->slug }} {{ $seasonAfter->year }}</a></li>
              @endforeach
            </ul>
          </div>
          <div class="filters__list is-right">
            <div class="toggle">
              <input type="checkbox" @change="filter" v-model="store.filters.sequels" id="filter__sequels">
              <label for="filter__sequels"></label>
            </div>
            <span>Show sequels</span>
            <a ref="trigger" :class="`popup__trigger ${ showFilter ? 'is-active' : ''}`" @click="toggleFilter">
            ...
            <filters :show-filter="showFilter" v-on:filter="filter"></filters>
          </div>
        </div>
      </div>
      <div class="seasonal__grid">
        <div class="container">
          <div class="row is-multiline">
            <tv_season v-for="tv_season in store.filtered" :key="tv_season.id" :season="store.season" :tv_season="tv_season"></tv_season>
          </div>
        </div>
      </div>
    </div>
